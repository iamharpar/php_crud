...
<html>
    ...
    <body class="mx-2">
        ...
        <?php
            session_start();
            $dbManagerObj = new DBManager();
            $results = $dbManagerObj->display();
            ?>
            ...
            if(isset($_POST['save'])) {
                ...
                $dbManagerObj->add($title, $summary);
                header("location:index.php");
            }elseif(isset($_POST['update'])) {
                $id = $_POST['id'];
                $title = $_POST['title'];
                $summary = $_POST['summary'];
                $dbManagerObj->update($id, $title, $summary);
                header("location: index.php");
            }

            if (isset($_GET['delete'])) {
                $id = $_GET['delete'];
                $dbManagerObj->delete($id);
                $_SESSION['message'] = "<script>alert('Article Deleted');</script>";
                header("location:index.php");
            }elseif(isset($_GET['accept'])) {
                $id = $_GET['accept'];
                $dbManagerObj->acceptArticle($id);
                header("location:index.php");
            }
        ?>
